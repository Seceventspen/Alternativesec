...
So, our Nmap scan has identified that TCP Port 22 SSH and Port 3128 Squid proxy are open. Now we have starting point!
...
Ok, so know we have a decent idea of what's going on with the web-server. Now that the proxy is enabled, I opened up a browser again and entered the target IP:
...
Taking this as a sign from the 'L33T' gods, I entered the old faithful combo of 'admin/admin'; however, this failed with the following error:
...
We trigger our reverse php shell by double clicking on it. This will then activate on our attacking host terminal where we set up the netcat listener:
...
*Note the 'sickos' user highlighted above.*
...
With the above info and using the discovered user 'sickos' I can now attempt to leverage the ssh service on the target host:
